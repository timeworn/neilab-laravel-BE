...
@section('scripts')
	<script>
		...
		function registerWithdraw(e){
			var superload_id = $(e).data('id');
			var tx_id = $("#withdraw_txid_"+superload_id).val();
			...
			$.ajax({
				...
				data: {
					"_token": "{{ csrf_token() }}",
					"superload_id": superload_id,
					"tx_id": tx_id,
				},
				success: function(data){
					if(data.success){
						$("#action_"+superload_id).html("Registered");
						$("#withdraw_txid_"+superload_id).attr("disabled", true);
					}else{
						alertError(data.msg);
					}
				},
